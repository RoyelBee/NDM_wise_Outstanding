...
order by AUDTORG asc, INVDATE desc, '0 - 3 days' desc""", fn.conn)
    ...
    # print(branch_wise_non_matured_credit_aging)
    branch_wise_cash_drop_aging.to_csv(r'./Data/branch_wise_cash_drop_aging.csv', index=False,
                                                header=True)

    all_return_df = lib.pd.read_sql_query("""
                select AUDTORG, sales.DPID, DPNAME.DPNAME, INVNUMBER, 
...
from (
select AUDTORG, DPID, TRANSTYPE, INVNUMBER, EXTINVMISC from OESalesDetails
...
group by AUDTORG, sales.DPID, DPNAME.DPNAME, INVNUMBER
...
order by ReturnPer desc""", fn.conn)
    ...
    # print(branch_wise_non_matured_credit_aging)
    all_return.to_csv(r'./Data/all_return.csv', index=False,
                                       header=True)
