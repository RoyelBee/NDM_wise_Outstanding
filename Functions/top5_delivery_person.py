...
def top5_delivery_persons_return():
    ...
    3) as BranchName, Sales.ReturnAmount from
                (select  DPID, AUDTORG,ISNULL(sum(case when TRANSTYPE<>1 then INVNETH *-1 end), .1) /ISNULL(sum(case when TRANSTYPE=1 then INVNETH end), .1)*100 as ReturnAmount from OESalesSummery
                where
