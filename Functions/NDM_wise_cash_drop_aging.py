...
def ndm_cash_drop():
    ...
    bar_two_lebel_array = [(kamrul_cash_drop[0] / sum(kamrul_cash_drop)) * 100,
                           ...
                           (kamrul_cash_drop[4] / sum(kamrul_cash_drop)) * 100,
                           (kamrul_cash_drop[5] / sum(kamrul_cash_drop)) * 100]
    ...
    bar_five_lebel_array = [(hafizur_cash_drop[0] / sum(hafizur_cash_drop)) * 100,
                            ...
                            (hafizur_cash_drop[4] / sum(hafizur_cash_drop)) * 100,
                            (hafizur_cash_drop[5] / sum(hafizur_cash_drop)) * 100]
    ...
    rects1 = lib.plt.bar(y_pos - width * 2, bar_one_lebel_array, width=.15, align='center', alpha=0.9, color='blue')
    rects2 = lib.plt.bar(y_pos - width, bar_two_lebel_array, width=.15, align='center', alpha=0.9, color='orange')
    rects3 = lib.plt.bar(y_pos, bar_three_lebel_array, width=.15, align='center', alpha=0.9, color='grey')
    rects4 = lib.plt.bar(y_pos + width, bar_four_lebel_array, width=.15, align='center', alpha=0.9, color='yellow')
    rects5 = lib.plt.bar(y_pos + width + width, bar_five_lebel_array, width=.15, align='center', alpha=0.9, color='red')
    ...
    labels = ['A- 0 t0 3 days', 'B- 4 to 10 days', 'C- 11 to 15 days', 'D- 16 to 30 days', 'E- 31 to 90 days',
              'F- 91 to 201 days']
